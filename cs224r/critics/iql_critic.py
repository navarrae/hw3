...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        # aggregate it later
        ### YOUR CODE HERE ###
        diff_negative = torch.zeros(diff.size()[0])
        torch.le(diff, 0, out=diff_negative)
        loss = torch.abs(self.iql_expectile - diff_negative) * (diff**2)
        return loss
        ### YOUR CODE HERE ###
    ...
    def update_v(self, ob_no, ac_na):
        ...
        # Compute the value prediction and the target for the value prediction.
        v_pred = self.v_net(ob_no).squeeze(1)

        with torch.no_grad():
            target_q_values = self.q_net_target(ob_no).gather(1, ac_na.unsqueeze(1)).squeeze(1)

        value_loss = self.expectile_loss(target_q_values - v_pred).mean()
        ### YOUR CODE HERE ###
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        loss = ((target_q_values - q_pred) ** 2).mean()
        ### YOUR CODE HERE ###
